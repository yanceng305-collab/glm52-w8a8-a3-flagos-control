...
| D-062 | Official文档定义的A3 release tag仍为`quay.io/ascend/vllm-ascend:v0.24.0rc1-a3`，但当前未建立可用artifact；A2允许exact digest`quay.io/ascend/vllm-ascend@sha256:1c36469fe1cd2335850eb2318bd3562471c34d5fd8f9f2affb0afc745ce39585`作provisional carrier | Required artifact identity / execution paused | 该digest身份仅为official `releases/v0.24.0rc` A3 nightly，不得称为rc1 release image；actual tuple已由latest run记录，但不自动授权重试 | official artifact变化或用户重新授权A2 |
| D-063 | A2 tiny runtime smoke允许共享NPU 12+13，不再要求完全空闲pair | Required boundary / execution paused | 只允许极小`torch_npu` tensor与FlagOS Dispatch operator smoke；禁止模型加载、vLLM serve、HCCL/TP、KV cache、完整WorkerFL/ModelRunnerFL runtime、benchmark/profile和大tensor；共享资源状态恶化立即STOP | 用户重新授权A2且环境gate可执行 |
| D-064 | FlagTree与carrier内compiler/provider共享namespace时必须形成single coherent provider | Required runtime gate / refined by D-074 | Python3.11-first路线目标为FlagTree`0.6.1+ascend3.5`；允许在disposable环境重建provider，但必须审计distribution/RECORD/module/native/backend ownership | D-074 task结果 |
...
| D-069 | A2技术排障允许与任务相关的公开/官方网络、registry、package index、CI artifact和合理镜像访问 | Required / refined by D-074 | 不再使用固定网络白名单；所有source/artifact必须记录provenance/hash，credential不得进入Evidence | 网络来源不可审计或出现供应链风险 |
| D-070 | Old Python3.12 A2对FlagGems的`no-deps`/禁止bootstrap限制 | **Superseded for D-074 Python3.11-first route** | D-074允许在disposable container/work中重装Python3.11对齐依赖、build wheel和临时patch；Host全局runtime与正式source保护仍Required | D-074 task结果 |
...
| D-074 | 当前integration-gap采用Python3.11-first：以exact vLLM-Ascend v0.24 carrier作不可变底座，在disposable范围切换/重建Python3.11用户态栈，不再要求保留或解决Python3.12 | Required / task Ready | 目标尽快闭合vLLM0.24 + torch/torch_npu + FlagTree`0.6.1+ascend3.5` + FlagGems + FL + Dispatch；允许合理联网、依赖重装、wheel构建和临时patch；不得修改Host全局runtime、覆盖base image或影响现有任务 | Python3.11 route PASS/STOP与可维护性证据 |

## Reclassified execution record

- `e9999d25`曾把A2 run `20260824T025250Z`写成D-073并把结果追加到active task。该内容是DeepSeek执行事实，不是技术路线Decision。
- Canonical immutable snapshot：[`results/A2-v024/20260824T025250Z.md`](results/A2-v024/20260824T025250Z.md)；原commit与Git历史完整保留。
- FlagTree Python 3.12 artifact与Python 3.11 carrier两条候选路线尚未获得Codex acceptance或用户批准，不在本轮形成新Decision。
