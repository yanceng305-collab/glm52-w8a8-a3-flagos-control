...
| D-073 | A2-v024 Phase A STOP at FlagTree；carrier Python 3.12.13与FlagTree Python 3.11 pre-built不兼容；py312状态Unresolved | Required / Evidence-backed | 2026-08-24执行：carrier digest/FL identity/FlagGems identity/vllm-ascend negative全部PASS；FlagTree install blocked：FlagOS official resource index unreachable (503)；源码build失败（cmake 4.4.0 > <4.0、离线build缺CUDA NVCC、rc1源码nvidia-toolchain-version.json缺失ptxas-blackwell key）；FlagTree 0.6.1+ascend3.5 Docker image pre-built为Python 3.11.15，pybind11 Python version mismatch阻止加载；**不结论”FlagTree不支持Python 3.12”**，py312=Unresolved；Evidence: `/data/tiankuan/zyg/evidence-a2-v024-20260824T025250Z` | FlagTree Python 3.12 pre-built可用或用户批准Python 3.11 carrier variant |
