...
**PAUSED by User Decision。** 当前 v0.24 GLM execution route停止推进；没有 GLM server execution、model task、Code task、benchmark或optimization获得授权。未下发、未执行的 `CANARY-V024-QWEN36-27B-TP2` 已 **SUPERSEDED / CANCELLED UNEXECUTED**。

当前工作转到独立的 [Qwen3.6-35B-A3B × FlagOS × Ascend A3/910C Validation Control](https://github.com/yanceng305-collab/qwen36-35b-a3b-a3-flagos-control)。未来恢复 GLM需要新的 User Decision，并先进行 vLLM 0.20.2 GLM contract review；本次暂停不自动批准新的 GLM baseline，也不开始 GLM port。
...
| Next Stage | **PAUSED**；恢复前先做 vLLM 0.20.2 GLM contract review |
| Canary status | **SUPERSEDED / CANCELLED UNEXECUTED**；never dispatched、no run、no Evidence、no Code change、no Acceptance |
...
阶段边界：baseline research、Code repo v0.24 migration与A2 clean-room gate已完成；旧v0.24 Python3.12 A2、copied-runtime Python3.11、全部 immutable Result/Evidence pointer/reconstruction/source identity和 host-local runtime-image记录完整保留为 historical/reference。暂停不修改任何既有 Acceptance。

## What Is NOT Done

Tiny runtime/operator/Dispatch smoke PASS **不等于**“GLM-5.2-W8A8 已可运行”。当前仍未完成：

- 已取消且从未执行的 `CANARY-V024-QWEN36-27B-TP2`没有model construction、real weight load、TP2 eager generation或Codex1 Acceptance；
...
| Historical cancelled 910C Canary contract | [CANARY-V024-QWEN36-27B-TP2.md](docs/glm52-w8a8-a3-flagos/tasks/CANARY-V024-QWEN36-27B-TP2.md) |
| Historical cancelled Canary prompt | [CODEX2-CANARY-V024-QWEN36-27B-TP2-PROMPT.md](docs/glm52-w8a8-a3-flagos/tasks/CODEX2-CANARY-V024-QWEN36-27B-TP2-PROMPT.md) |
