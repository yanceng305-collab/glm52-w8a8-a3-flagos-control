...
状态：**Active；A3-CP-A2-v024 Ready / not executed**
...
- A3 device contract：至少一个valid、完整空闲的two-logical-device pair，现场只读确认。
...
```text
Host image/topology/OC2 preflight
  -> exact v0.24 carrier (pull only if absent)
  -> disposable container on valid free pair
  -> uninstall vllm-ascend + fresh negative check
  -> uninstall triton/triton-ascend distributions
  -> install exact FlagTree + prove coherent provider
  -> install/confirm FlagGems v5.3.4
  -> install project/glm52-w8a8-v024 from writable staging
...
A2只在Ready task全部PASS且Evidence完整时进入Codex验收。任何package、provider、device pair、source identity或范围问题均STOP，不做手工清理或自动fallback。
