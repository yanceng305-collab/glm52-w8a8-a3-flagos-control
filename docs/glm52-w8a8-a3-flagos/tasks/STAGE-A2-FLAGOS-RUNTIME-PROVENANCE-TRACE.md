...
这是工程优先级调整，不是恢复“vllm-ascend image/package存在即违规”的旧规则。前置A2在一次性实验container内卸载vllm-ascend，只用于减少首次bring-up变量；official carrier中保留package并通过`VLLM_PLUGINS=fl`运行仍是合法路线。Post-Eager审计为Eager Correctness后的Deferred / On-demand支线，不阻塞Baseline Benchmark。
